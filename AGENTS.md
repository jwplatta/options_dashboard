...
- Source code lives in `src/options_dashboard/`, with the Streamlit entry point at `src/options_dashboard/app.py`.
...
- `uv run streamlit run src/options_dashboard/app.py`: run the Streamlit dashboard locally.
...
- Keep Streamlit UI logic in `app.py`; add new modules under `src/options_dashboard/` for reusable logic.
